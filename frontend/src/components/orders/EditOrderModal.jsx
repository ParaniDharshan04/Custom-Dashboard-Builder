...
import { PRODUCTS, COUNTRIES, STATUSES, CREATED_BY } from '../../utils/validators';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center animate-fade-in">
      ...
      <div className={cn(
        ...
        <div className={cn(
          ...
          <h2 className={cn("text-lg font-semibold", isDarkMode ? "text-gray-200" : "text-gray-800")}>Edit Order</h2>
          <button onClick={onClose} className={cn("transition", isDarkMode ? "text-gray-500 hover:text-gray-300" : "text-gray-400 hover:text-gray-600")} id="edit-order-close">
            <X size={20} />
          </button>
        </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input {...register('first_name')} className={inputClass('first_name')} id="edit-first-name" />
                {errors.first_name && <p className="text-xs text-red-500 mt-1">{errors.first_name.message}</p>}
              </div>
              <div>
                <label className={labelClass}>Last name *</label>
                <input {...register('last_name')} className={inputClass('last_name')} id="edit-last-name" />
                {errors.last_name && <p className="text-xs text-red-500 mt-1">{errors.last_name.message}</p>}
              </div>
              <div>
                <label className={labelClass}>Email *</label>
                <input {...register('email')} type="email" className={inputClass('email')} id="edit-email" />
                {errors.email && <p className="text-xs text-red-500 mt-1">{errors.email.message}</p>}
              </div>
              <div>
                <label className={labelClass}>Phone number *</label>
                <input {...register('phone_number')} className={inputClass('phone_number')} id="edit-phone" />
                {errors.phone_number && <p className="text-xs text-red-500 mt-1">{errors.phone_number.message}</p>}
              </div>
              <div className="md:col-span-2">
                <label className={labelClass}>Street Address *</label>
                <input {...register('street_address')} className={inputClass('street_address')} id="edit-street" />
                {errors.street_address && <p className="text-xs text-red-500 mt-1">{errors.street_address.message}</p>}
              </div>
              <div>
                <label className={labelClass}>City *</label>
                <input {...register('city')} className={inputClass('city')} id="edit-city" />
                {errors.city && <p className="text-xs text-red-500 mt-1">{errors.city.message}</p>}
              </div>
              <div>
                <label className={labelClass}>State / Province *</label>
                <input {...register('state_province')} className={inputClass('state_province')} id="edit-state" />
                {errors.state_province && <p className="text-xs text-red-500 mt-1">{errors.state_province.message}</p>}
              </div>
              <div>
                <label className={labelClass}>Postal code *</label>
                <input {...register('postal_code')} className={inputClass('postal_code')} id="edit-postal" />
                {errors.postal_code && <p className="text-xs text-red-500 mt-1">{errors.postal_code.message}</p>}
              </div>
              <div>
                <label className={labelClass}>Country *</label>
                <select {...register('country')} className={inputClass('country')} id="edit-country">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                ...
                <select {...register('product')} className={inputClass('product')} id="edit-product">
              ...
              <div>
                ...
                <input type="number" {...register('quantity', { valueAsNumber: true })} min="1" className={inputClass('quantity')} id="edit-quantity" />
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" step="0.01" {...register('unit_price', { valueAsNumber: true })} className={cn(inputClass('unit_price'), 'pl-7')} id="edit-unit-price" />
                </div>
              ...
              <div>
                ...
                <select {...register('status')} className={inputClass('status')} id="edit-status">
                  {STATUSES.map((s) => (
              ...
              <div className="md:col-span-2">
                ...
                <select {...register('created_by')} className={inputClass('created_by')} id="edit-created-by">
